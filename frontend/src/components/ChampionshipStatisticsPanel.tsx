      ...
      {(["goals", "assists"] as const).map((kind) => (
        <section key={kind} className="card overflow-hidden">
          <div className="border-b border-base-700 px-4 py-3">
            <h2 className="font-semibold text-slate-100">{kind === "goals" ? "Artilharia por jogador" : "Assistências por jogador"}</h2>
            <p className="mt-1 text-xs text-slate-500">Ranking individual acumulado nas partidas encerradas · {label(kind)}</p>
          </div>
          {ranking(kind).length === 0 ? <p className="px-4 py-8 text-center text-sm text-slate-500">Nenhuma estatística lançada ainda.</p> : (
            <div className="divide-y divide-base-700">
              {ranking(kind).slice(0, 10).map((row, index) => {
                ...
                return <div key={row.playerId} className="flex items-center gap-3 px-4 py-3"><span className="w-5 text-sm font-bold text-slate-500">{index + 1}</span><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-cyan-400">Jogador</p><p className="truncate text-sm font-medium text-slate-200">{row.playerName}</p><p className="truncate text-xs text-slate-500">Time: {row.teamName}</p></div><span className="text-lg font-bold text-accent-400">{value}</span></div>;
              })}
            </div>
          )}
        </section>
